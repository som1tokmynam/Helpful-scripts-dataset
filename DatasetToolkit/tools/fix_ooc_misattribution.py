...
def is_junk(turn):
    ...
    if turn.get('from') == 'gpt' and value_lower.endswith(':') and len(value_stripped.split()) < 3: return True
    return False
            ...
            for i, line in enumerate(infile):
                ...
                try:
                    ...
                    for turn in pass1_turns:
                        value = turn.get('value', '')
                        # Check if the turn needs splitting (contains the pattern but doesn't start with it)
                        if re.search(split_pattern, value):
                            # Split the value by the markers, keeping the markers themselves
                            parts = re.split(split_pattern, value)
                            
                            # The first part is special, it doesn't have a marker prefixed
                            if parts[0].strip():
                                pass2a_turns.append({'from': turn.get('from'), 'value': parts[0].strip()})
                            
                            # Process the rest of the parts, which come in pairs (marker, content)
                            for j in range(1, len(parts), 2):
                                marker = parts[j]
                                content = parts[j+1]
                                if marker and content.strip():
                                    # Reconstruct the turn's value with the marker it was split by
